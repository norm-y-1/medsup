...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="card p-4 flex flex-col gap-2 cursor-pointer" data-testid="product-card">
      <div className='overflow-hidden rounded-2xl w-[323px] h-[242px]'>
        <Skeleton width='323px' height='242px' condition={product.image ? isLoaded : true}>
          <img
            src={product.image || NoPhoto}
            alt={product.name}
            className="aspect-[4/3] w-full object-contain rounded-xl hover:scale-110 transition duration-300 ease"
            loading="lazy"
          />
        </Skeleton>
      </div>
